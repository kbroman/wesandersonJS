...
var bgrect, circhoffset, dark_colors, has_dark, height, hoffset, hscale, indent, index, max_length, n_per_col, pal, palettes, radius, svg, text, textwidth, vscale, vscaleL, vscaleR, width;
...
hoffset = function(d) {
  ...
  return indent + 25 + width / 2;
};

circhoffset = function(d) {
  if (d < n_per_col) {
    return indent;
  }
...
for (index in palettes) {
  ...
  svg.append("g").attr("id", pal).selectAll("empty").data(wes_palettes[pal]).enter().append("circle").attr("cy", vscale(index)).attr("cx", function(d, i) {
    return circhoffset(index) + hscale(i);
