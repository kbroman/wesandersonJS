...
var bgrect, dark_colors, has_dark, height, hoffset, hscale, indent, index, max_length, n_per_col, pal, palettes, radius, svg, text, textwidth, vscale, vscaleL, vscaleR, width;
...
bgrect = svg.append("rect").attr("height", height).attr("width", width).attr("fill", "white").on("click", function(d) {
...
vscaleL = d3.scale.ordinal().domain(d3.range(n_per_col)).rangePoints([0, height], 1);

vscaleR = d3.scale.ordinal().domain(d3.range(n_per_col).map(function(d) {
  return d + n_per_col;
...
hscale = d3.scale.ordinal().domain(d3.range(max_length)).rangePoints([textwidth, width / 2], 2);
